...
class ActorCritic(nn.Module):
    ...
    def act(self, state):
        ...
        return action.cpu().data.numpy(), action_logprob.detach()
...
def main():
    ...
    min_action_std = 0.01
    save_model_freq = [2500*num_step, 5000*num_step, 10000*num_step, 20000*num_step]
    ...
    for i in tqdm.tqdm(range(num_iteration)):
        ...
        for j in range(num_step):
            for node in range(env.node_num):
                ...
                print("action", act)
                agent.addBufferSA(torch.FloatTensor(action).to(device), state, action_prob)
            ...
            # # save model weights
            # if (step + 1) in save_model_freq:
            # # if step == 0:
            #     # print("--------------------------------------------------------------------------------------------")
            #     # print("saving model at : " + checkpoint_path)
            #     record = (step + 1) / num_step
            #     checkpoint_p = "path_32/PPO_seed_agent"
            #     for node in range(env.node_num):
            #         checkpoint_path = checkpoint_p + str(node+1) + '_' + str(record) + '.pth'
            #         agents[node].save(checkpoint_path)
